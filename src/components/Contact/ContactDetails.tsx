        ...
        <section className='w-full rounded border border-[#E1E1E1] p-10'>
            <div className="flex flex-col gap-5">
                ...
                <div className='flex flex-col gap-10 mt-5'>
                    {
                        contactDetails.map((contact, index) => (
                            <div key={index} className='flex flex-row gap-5 items-start'>
                                <Image src={contact.image} alt={contact.label} width={20} height={20} />
                ...
                <div className='flex flex-col gap-5 mt-10'>
                    <h2 className='font-bold text-custom-black'>Follow Us</h2>
                    <div className="flex w-full flex-row gap-10  pb-5">
                        {
                            socialMedia.map((key, index) => (
                                <Link key={index} href={'#'} className='rounded-full bg-white  flex items-center justify-center'>
                                    <Image src={key} alt='social-media' width={15} height={15} />
                                </Link>
                            ))
                        }
                    </div>
